		   ...
		   int* model, int* version)
{
   ...
   *model = 0;
...
#ifdef _NCOPEN_DEBUG_X
   printf("NC_check_file_type called with %s\n", path);
#endif
...
#endif /* USE_PARALLEL */
   {
	  ...
	  {
#ifdef _NCOPEN_DEBUG_X
		  printf("fstat failed checking errno %d\n", errno);
#endif
		  if (errno == EOVERFLOW)
			  sizeok = 1;
		  else
	  ...
	  if (!sizeok) //The file is too big for stat
	  {
		  if (st.st_size < MAGIC_NUMBER_LEN)
		  {
#ifdef _NCOPEN_DEBUG_X
			  printf("fstat size less than MAGIC_NUMBER_LEN\n");
#endif
			  fclose(fp);
			  status = NC_ENOTNC;
			  goto done;
...
#ifdef _NCOPEN_DEBUG_X
   printf("NC_open Called with %s mode %d\n", path, cmode);
#endif

   if(!nc_initialized) {
#ifdef _NCOPEN_DEBUG_X
	   printf("Initializing NC\n");
#endif
	   stat = NC_initialize();
      if(stat) return stat;
      /* Do local initialization */
      ...
      nc_initialized = 1;
#ifdef _NCOPEN_DEBUG_X
	  printf("NC Initialization successfull\n");
#endif
   }
...
#ifdef _NCOPEN_DEBUG_X
   printf("NC_testurl returned %d\n", isurl);
#endif
   if (isurl)
   {
	   model = NC_urlmodel(path);
#ifdef _NCOPEN_DEBUG_X
	   printf("NC_urlmodel returned %d\n", model);
#endif
   }
   else 
   {
      ...
      stat = NC_check_file_type(path,useparallel,mpi_info,
				&model,&version);
#ifdef _NCOPEN_DEBUG_X
	  printf("NC_check_file_type returned %d\n", stat);
#endif
	  ...
	  if(stat == NC_NOERR) 
	  {
		  ...
		  {
#ifdef _NCOPEN_DEBUG_X
			  printf("NC_check_file_type model was %d\n", model);
#endif
			  return NC_ENOTNC;
		  }
      } 
